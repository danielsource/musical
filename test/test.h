...
#define RUN_TEST(test)                                                        \
  do {                                                                        \
    puts(ANSI_COLOR_CYAN ANSI_COLOR_BOLD "RUNNING" ANSI_COLOR_RESET           \
                                         ": " #test);                         \
    char *message = test();                                                   \
    tests_run++;                                                              \
    if (message) {                                                            \
...
#define RUN_TEST_SUITE(run_all_tests_func)                                    \
  do {                                                                        \
    puts(ANSI_COLOR_CYAN ANSI_COLOR_BOLD "TEST SUITE" ANSI_COLOR_RESET        \
                                         ": " __FILE__);                      \
    ...
    printf(ANSI_COLOR_CYAN ANSI_COLOR_BOLD "TESTS RUN" ANSI_COLOR_RESET       \
                                           ": %d\n\n",                        \
           tests_run);                                                        \
    return result != 0;                                                       \
  } while (0)
